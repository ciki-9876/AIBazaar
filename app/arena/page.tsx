...
export default function ArenaPage() {
  ...
  const [enemyInspection, setEnemyInspection] =
    useState<EnemyInspection | null>(null);
  const [ampLane, setAmpLane] = useState<number | null>(null);
  ...
  const frame = result.frames[
    Math.max(0, Math.min(cursor, result.frames.length - 1))
  ] as ArenaFrame;
  const filtered = ARENA_CARDS.filter(
    (card) =>
      (kind === 'all' || card.kind === kind) &&
      `${card.name}${card.text}`.includes(search.trim()),
  );
  ...
  useEffect(() => {
    const id = setTimeout(() => {
      try {
        ...
        setNotice('本机对局档案未能读取；仍可开始新对局并导出记录。');
      }
    }, 0);
  ...
  useEffect(() => {
    ...
    const tick = (time: number) => {
      ...
      previous = time;
      setCursor(replayFrameIndex(clock.current, result.frames.length));
  ...
  const toggleFullscreen = async () => {
    if (expanded) {
      ...
      setExpanded(true);
      // Embedded browsers may deny native fullscreen; the viewport mode remains usable.
      await page.current?.requestFullscreen?.().catch(() => {});
    }
  };

  const inspectEnemy = (inspection: EnemyInspection) => {
    setPlaying(false);
    setEnemyInspection(inspection);
  };

  const chooseSlot = (at: number) => {
    if (phase !== 'build') return;
    const occupant = player.find(
      (card) => card.at <= at && at < card.at + cardDef(card.id).size,
    );
    if (occupant) {
      setPlayer((board) => board.filter((card) => card.uid !== occupant.uid));
      setSnapshot(null);
      return;
    }
    if (!selected) {
      setNotice('先从左侧选择一张牌，再点击己方空格。');
      return;
    }
    try {
      const next = placeArenaCard(
        player,
        selected,
        at,
        `player-${serial.current++}`,
      );
      setPlayer(next);
      setSnapshot(null);
      setDetail(selected);
    } catch (error) {
      setNotice(error instanceof Error ? error.message : '放置失败');
    }
  };
  ...
  const loadMatch = (match: ArenaMatch) => {
    ...
    clock.current = res.duration;
  };
  const viewed = arenaCard(detail ?? selected ?? '');
  const inspectedCard =
    enemyInspection?.kind === 'card'
      ? arenaCard(enemyInspection.id)
      : undefined;
  const inspectedAmp =
    enemyInspection?.kind === 'amplifier'
      ? amplifier(enemyInspection.id)
      : undefined;
  return (
    <main
      ref={page}
      className={`arena-page ${expanded ? 'arena-expanded' : ''}`}
    >
      <header className="arena-header">
        <div className="arena-brand">
          ...
          <span className="arena-mark">
            F9 <i>/</i> TACTICAL LAB
          </span>
          <span className="arena-version">对战博弈模拟 · 实验规则 v0</span>
        </div>
      ...
      <div className="arena-layout">
        <aside className="arena-catalog">
          <div className="arena-panel-title">
            <span>01 / 武装库</span>
            <strong>{ARENA_CARDS.length} 张实验牌</strong>
          </div>
          ...
          <div className="arena-filters">
            {['all', ...Object.keys(kinds)].map((key) => (
              <button
                key={key}
                className={kind === key ? 'active' : ''}
                onClick={() => setKind(key)}
              >
                {key === 'all' ? '全部' : kinds[key]}
              </button>
            ))}
          </div>
          <div className="arena-card-list">
            {filtered.map((card) => (
              <button
                key={card.id}
                className={`arena-card rarity-${card.rarity} ${selected === card.id ? 'selected' : ''}`}
                onClick={() => {
                  setSelected(card.id);
                  setDetail(card.id);
                }}
              ...
              >
                <span className="arena-card-glyph">
                  {String(card.number).padStart(2, '0')}
                </span>
                <span className="arena-card-copy">
                  <strong>{card.name}</strong>
                  <small>
                    {kinds[card.kind]} · {card.cd ? `${card.cd} 秒` : '被动'}
                  </small>
                </span>
                <span className="arena-card-tail">
                  <b>{card.size}格</b>
                  <small>{RARITIES[card.rarity]}</small>
                </span>
              </button>
            ))}
          </div>
        ...
        <section className="arena-main">
          <div className="arena-stage">
            {phase === 'result' && currentMatch && (
              <output className="arena-stage-outcome">
                {currentMatch.summary.winner === 0
                  ? '你赢了'
                  : currentMatch.summary.winner === 1
                    ? '对手取胜'
                    : '限时平局'}{' '}
            ...
            <button
              className="arena-fullscreen"
              onClick={() => void toggleFullscreen()}
              aria-label={expanded ? '退出全屏' : '全屏观看'}
            >
            ...
            <Suspense
              fallback={
                <div className="arena-stage-loading">战术室正在点亮…</div>
              }
            >
              <Scene
                duel={duel}
            ...
            <div className="arena-stage-top">
              <div>
                <small>模拟对手 / {lineup.id.toUpperCase()}</small>
                <strong>{lineup.title}</strong>
                <p>{lineup.thesis}</p>
              </div>
              <span>
                宿主 {Math.round(frame.hp[1])} / {duel.maxHp[1]}
              </span>
            </div>
            {ready && (
              <div className="arena-scene-labels">
                {[0, 1].flatMap((side) =>
                  (side ? duel.enemy : duel.player).map((card) => {
                    const a = anchors[side * 9 + card.at],
                      b =
                        anchors[side * 9 + card.at + cardDef(card.id).size - 1];
                    if (!a || !b) return null;
                    const label = (
                      <>
                        <span>{cardDef(card.id).name}</span>
                        <small>
                          {frame.cd[side][card.at]
                            ? `${Math.max(0, frame.cd[side][card.at] - frame.timers[side][card.at]).toFixed(1)}s`
                            : '被动'}
                        </small>
                      </>
                    );
                    return side ? (
                      <button
                        key={card.uid}
                        type="button"
                        className="arena-scene-card enemy"
                        style={{
                          left: (a.x + b.x) / 2,
                          top: Math.min(a.y, b.y) - 22,
                        }}
                        onClick={() =>
                          inspectEnemy({
                            kind: 'card',
                        ...
                        aria-label={`查看敌方${LANES[Math.floor(card.at / 3)]}${cardDef(card.id).name}的效果`}
                      >
                        {label}
                      </button>
                    ) : (
                      <div
                        key={card.uid}
                        className="arena-scene-card own"
                        style={{
                          left: (a.x + b.x) / 2,
                          top: Math.min(a.y, b.y) - 22,
                        }}
                ...
                {board.barriers.map((a, index) => {
                  ...
                  return (
                    <div
                      key={index}
                      className={`arena-scene-shield ${side ? 'enemy' : 'own'} ${b.broken ? 'broken' : ''}`}
            ...
            <div className="arena-stage-bottom">
              <span>
                己方宿主{' '}
                <b>
                  {Math.round(frame.hp[0])} / {duel.maxHp[0]}
                </b>
              </span>
              <span>
                {LANES.map(
                  (lane, i) =>
                    `${lane} ${frame.barriers[0][i].broken ? '破屏' : Math.round(frame.barriers[0][i].hp)}`,
                ).join('  /  ')}
              </span>
            </div>
            <div className="arena-actions arena-stage-controls">
              {phase === 'build' ? (
                <button className="arena-primary" onClick={start}>
                  <Swords size={17} />
                  开始对战并记录
                </button>
              ) : (
                <>
                  <button
                    onClick={() => {
                      ...
                      setPlaying(!playing);
                      if (phase === 'result') setPhase('fight');
                    }}
                  >
                    <span>
                  ...
                  <label className="arena-timeline">
                    <input
                      type="range"
                      min={0}
              ...
              {phase !== 'build' && (
                <button
                  ...
                  onClick={() => {
                    if (expanded) void toggleFullscreen();
          ...
          <div className="arena-workbench">
            <div className="arena-workbench-heading">
              <div>
                <small>02 / 布阵台</small>
                <h1>
                  {phase === 'build' ? '选择牌，再点击己方空格' : '战斗记录'}
                </h1>
              </div>
              <span>9 格全开放 · 同名至多 2 张 · 每路独立增幅器</span>
            </div>
            {viewed && (
              <div className="arena-selected-brief">
                <strong>{viewed.name}</strong>
                <span>{viewed.text}</span>
                <small>
                  {RARITIES[viewed.rarity]} · {viewed.size} 格 ·{' '}
                  {viewed.cd ? `${viewed.cd} 秒周期` : '被动'}
                </small>
                <EffectHelp id={viewed.id} />
              </div>
            )}
            <FormationBoard
              player={player}
              enemy={lineup.cards}
              playerAmps={playerAmps}
              enemyAmps={lineup.amps}
              editable={phase === 'build'}
              onSlot={chooseSlot}
              onAmp={setAmpLane}
              onInspect={inspectEnemy}
            />
            {playerAmps.some(Boolean) && (
              <div className="arena-amp-summary">
                {playerAmps.map(
                  (id, lane) =>
                    id && (
                      <p key={lane}>
                        <b>
                          {LANES[lane]} · {amplifier(id)?.name}
                        </b>
                        {amplifier(id)?.text}
                      </p>
                    ),
            ...
            {phase === 'result' && currentMatch && (
              <div className="arena-result">
                ...
                <span>
                  己方 {Math.round(frame.hp[0])} HP · 对手{' '}
                  {Math.round(frame.hp[1])} HP · {result.duration.toFixed(1)} 秒
                </span>
                {currentMatch.summary.winner === 0 && (
                  <button onClick={counter}>申请调整对手阵容 →</button>
                )}
              </div>
            )}
            {phase === 'result' && currentMatch && (
              <div className="arena-postmortem">
                <div>
                  <small>宿主承伤</small>
                  <strong>
                    我方 {Math.round(currentMatch.summary.damageToHost[0])} /
                    对手 {Math.round(currentMatch.summary.damageToHost[1])}
                  </strong>
                </div>
                <div>
                  <small>屏障承伤</small>
                  <strong>
                    我方 {Math.round(currentMatch.summary.damageToBarrier[0])} /
                    对手 {Math.round(currentMatch.summary.damageToBarrier[1])}
                  </strong>
                </div>
                <div>
                  <small>对手首度破屏</small>
                  <strong>
                    {LANES.map(
                      (lane, i) =>
                        `${lane} ${currentMatch.summary.brokenAt[1][i] === null ? '未破' : `${currentMatch.summary.brokenAt[1][i]}s`}`,
                    ).join(' · ')}
                  </strong>
                </div>
              </div>
        ...
        <aside className="arena-intel">
          <section className="arena-intel-block">
            <div className="arena-panel-title">
              <span>03 / 战术情报</span>
            </div>
            <h2>{viewed ? viewed.name : lineup.title}</h2>
            <p>{viewed ? viewed.text : lineup.thesis}</p>
            {viewed && <EffectHelp id={viewed.id} />}
            {viewed ? (
              <div className="arena-detail-meta">
                <span>{RARITIES[viewed.rarity]}</span>
                <span>{viewed.size} 格</span>
                <span>{viewed.cd ? `${viewed.cd}s 周期` : '被动'}</span>
              </div>
            ) : null}
          </section>
          <section className="arena-intel-block">
            <div className="arena-panel-title">
              <span>对手阵容 · 点击查看效果</span>
              <strong>{lineup.cards.length} 件</strong>
            </div>
            {LANES.map((lane, l) => (
              <div key={lane} className="arena-opponent-lane">
                <small>{lane}</small>
                {lineup.amps[l] && (
                  <button
                    type="button"
                    className="arena-opponent-amp"
                    onClick={() =>
                      inspectEnemy({
                        kind: 'amplifier',
                        id: lineup.amps[l]!,
                  ...
                  {lineup.cards
                    ...
                    .map((c) => (
                      <button
                        ...
                        type="button"
                        onClick={() =>
                          inspectEnemy({ kind: 'card', id: c.id, at: c.at })
                        }
                      >
                        {cardDef(c.id).name} <i>{cardDef(c.id).size}格</i>
                      </button>
                    ))}
                </div>
              </div>
          ...
          <section className="arena-intel-block arena-history">
            <div className="arena-panel-title">
              <span>04 / 对局档案</span>
              <strong>{archive.matches.length} 场</strong>
            </div>
            <p>
              每场保存完整布阵、增幅器、规则版本、胜负、伤害及破屏时间。点击可复现回放。
            </p>
            <div className="arena-history-list">
              {[...archive.matches]
                ...
                .slice(0, 20)
                .map((match) => (
                  <button key={match.id} onClick={() => loadMatch(match)}>
                    <span>
                      {match.duel.name}
                      <small>
                        {new Date(match.createdAt).toLocaleString('zh-CN')}
                      </small>
                    </span>
                    <b>
                      {match.summary.winner === 0
                        ? '胜'
                        : match.summary.winner === 1
                          ? '负'
                          : '平'}
                    </b>
                  </button>
            ...
            <div className="arena-archive-actions">
              <button onClick={exportArchive}>
                <Download size={15} /> 导出 JSON
              </button>
              <button onClick={() => file.current?.click()}>
                <BookOpen size={15} /> 导入 JSON
              </button>
              <input
                ref={file}
                type="file"
          ...
          <div className="arena-note">
            实验牌的数值与规则仍在校准。美术物件为已完成原型资产的暂用映射；结算只依赖规则数据。
          </div>
        </aside>
      </div>
      {ampLane !== null && (
      ...
      {enemyInspection && (
        <ArenaDialog
          title="敌方效果详情"
          onClose={() => setEnemyInspection(null)}
        >
          <small className="arena-inspection-index">
            {
              LANES[
                enemyInspection.kind === 'card'
                  ? Math.floor(enemyInspection.at / 3)
                  : enemyInspection.lane
              ]
            }{' '}
            / {inspectedCard ? '卡牌' : '屏障增幅器'}
          </small>
          <h2>{inspectedCard?.name ?? inspectedAmp?.name}</h2>
          <p>{inspectedCard?.text ?? inspectedAmp?.text}</p>
          {inspectedCard && (
            <div className="arena-inspection-meta">
          ...
          {inspectedAmp && (
            <div className="arena-inspection-meta">
              <span>{RARITIES[inspectedAmp.rarity]}</span>
              <span>屏障击破后失效</span>
              {enemyInspection.kind === 'amplifier' &&
                !frame.amplifierActive[1][enemyInspection.lane] && (
                  <span>当前已失效</span>
                )}
            </div>
          )}
          <EffectHelp id={enemyInspection.id} />
          <div className="arena-inspection-foot">
            对手效果公开 · 查看时暂停播放
          </div>
        </ArenaDialog>
      )}
