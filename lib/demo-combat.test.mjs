...
test('combat revision: damage passes through target armor then owner shield; target cards have no health or destruction', () => {
  ...
  assert.ok(
    r.frames.some((f) =>
      f.hits.some((h) => h.targetUid === 'e' && h.shieldAbsorbed > 0),
    ),
  );
  assert.ok(
    r.frames
...
test('combat revision: front position and individual armor growth alter transmitted damage without adding target HP', () => {
...
test('combat revision: miracle echo uses the same target and armor without recursive echoes', () => {
  ...
  assert.ok(hits.every((h) => h.targetUid === 's'));
});
