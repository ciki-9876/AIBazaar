...
test('inventory and board moves are reversible; invalid cross-lane placement is atomic', () => {
  let s = start();
  s = act(s, { type: 'move', id: 'starter-nailer', to: 'bag', slot: 4 });
  const before = structuredClone(s);
  assert.throws(() =>
    act(s, { type: 'move', id: 'starter-nailer', to: 'board', at: 2 }),
  );
  assert.deepEqual(s, before);
  s = act(s, { type: 'move', id: 'starter-nailer', to: 'board', at: 3 });
  assert.equal(s.items.find((x) => x.uid === 'starter-nailer').zone, 'board');
  s = act(s, { type: 'move', id: 'starter-nailer', to: 'bag', slot: 4 });
  assert.equal(s.items.find((x) => x.uid === 'starter-nailer').slot, 4);
