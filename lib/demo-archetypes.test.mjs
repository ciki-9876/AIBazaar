...
test('burst expires and exposed damage checks the barrier on impact', () => {
  ...
  assert.equal(gap[0].value, 10);
  assert.equal(gap[0].healthLoss, 5);
  assert.equal(gap[1].value, 22);
});
test('buffer applies only on its intact lane, does not stack, and recoil stores actual direct barrier damage', () => {
  const r = simulateDuel(
    duel([card('knife', 0, 'attack')], [card('recoil', 0), card('rubber', 2)]),
  );
  assert.equal(hits(r, 'attack')[0].blocked, 8);
  assert.equal(hits(r, 'attack')[0].barrierAbsorbed, 4);
  assert.equal(hits(r, 'recoil')[0].raw, 26);
  const off = simulateDuel(
    duel([card('knife', 3, 'attack')], [card('rubber', 2)]),
  );
...
test('corrosion ticks after impact, bypasses buffer, reduces repair ceiling, persists through a broken barrier, caps at 12', () => {
  ...
  assert.equal(r.frames.find((f) => f.time === 6).corrosion[1][0], 0);
  assert.equal(r.frames.find((f) => f.time === 6.25).corrosion[1][0], 4);
  const tick = hits(r, 'acid').find((h) => h.periodic);
  assert.equal(tick.value, 4);
  ...
  assert.equal(hits(r, 'recoil')[0].raw, 24);
});
...
test('same-tick damage and repair do not give the first board an advantage; three archetypes form the nominal counter cycle', () => {
  for (const a of ARCHETYPES)
    for (let p = 0; p < 6; p++) {
      const d = archetypeDuel(a.id, a.beats, 0, 0, 0, p),
        r = simulateDuel(d),
        reverse = simulateDuel({ ...d, player: d.enemy, enemy: d.player });
      assert.equal(
        r.winner,
        0,
        `${a.id} should counter ${a.beats} at nominal growth`,
      );
