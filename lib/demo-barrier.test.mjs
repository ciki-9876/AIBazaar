...
test('barriers absorb only their lane; breaking hit spills to host; cards are never damage targets', () => {
  ...
  assert.equal(hits[0].barrierAbsorbed, 20);
  assert.equal(hits[0].healthLoss, 0);
  assert.equal(hits[1].barrierAbsorbed, 10);
  assert.equal(hits[1].healthLoss, 10);
  assert.equal(hits[2].targetUid, 'host-1-lane-0');
  assert.ok(
    r.frames.every(
      (f) => f.barriers[1][1].hp === 30 && f.barriers[1][2].hp === 30,
    ),
  );
  assert.ok(
    r.frames
