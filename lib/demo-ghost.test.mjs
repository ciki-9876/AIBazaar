...
test('ghost: lethal card damage disables targeting, activations and in-flight effects until full revival', () => {
  ...
  assert.equal(reviveTimeOf({ ...e, level: 5 }), reviveTimeOf(e));
  assert.equal(reviveTimeOf(e, 2), 7.5);
  assert.equal(reviveTimeOf(e, 3), 9);
  const second = r.frames.find((f) => f.cards.e.deaths === 2);
  assert.ok(second);
  assert.equal(second.cards.e.reviveAt - second.time, 7.5);
});
