...
test('progression: facilities unlock with level and consumable scanners are available early', () => {
  let s = start();
  ...
  assert.throws(() => act(s, { type: 'grow', id: 'starter-gapblade' }), /Lv.2/);
  ...
  s.level = 1;
  assert.equal(unlockedItem(s, 'scanner'), true);
});
const card = (uid, flightTime, id = 'slingshot') => ({
  uid,
  flightTime,
  id,
  at: 0,
...
test('projectiles: shield arrives after launch and every impact is delivered once', () => {
  const result = simulateDuel(
    duel([card('shield', 1.5, 'rubber')], [card('enemy', 1)], [1000, 1000]),
  );
...
test('hit types: instant knife lands at 3s without a projectile; slingshot lands after flight; simultaneous blades draw', () => {
  const direct = duel([{ ...card('blade', 2, 'gapblade') }], [], [100, 100]);
  direct.barrierHp = [
    [1, 1, 1],
    [1, 1, 1],
  ];
  const instant = simulateDuel(direct);
  assert.equal(
    instant.frames.find((f) => f.hits.some((h) => h.sourceUid === 'blade'))
      .time,
    3,
  );
  assert.ok(
    instant.frames.every(
      (f) => !f.projectiles.some((p) => p.sourceUid === 'blade'),
    ),
  );
  const flying = simulateDuel({
    ...direct,
    player: [card('sling', 1.25, 'slingshot')],
  });
  assert.equal(
    flying.frames.find((f) =>
      f.projectiles.some((p) => p.sourceUid === 'sling'),
    ).time,
    3,
  );
  assert.equal(
    flying.frames.find((f) => f.hits.some((h) => h.sourceUid === 'sling')).time,
    4.25,
  );
  const simultaneous = simulateDuel({
    ...direct,
    maxHp: [9, 9],
    enemy: [card('enemy', 0.5, 'gapblade')],
  });
  assert.equal(simultaneous.winner, -1);
  assert.equal(simultaneous.duration, 3);
  assert.deepEqual(simulateDuel(JSON.parse(JSON.stringify(direct))), instant);
});
