...
const duel = (player, enemy = [card('knife', 0, 'enemy')]) => ({
  player,
  enemy,
  maxHp: [5000, 5000],
  weather: 0,
...
test('phase1 descriptions separate present/unlocked/future and match actual third hits at each quality', () => {
  for (const id of ['knife', 'brick', 'bottle', 'wire', 'shelter'])
    for (const quality of [0, 1, 2]) {
      const c = { ...card(id, 0), quality };
      const d = describeCard(c);
      assert.ok(
        ![...d.innate, ...d.unlocked].some((x) => x.includes('解锁：')),
      );
      assert.equal(d.future.length, 2 - quality);
      if (!quality) assert.equal(d.unlocked.length, 0);
      const sim = simulateDuel(duel([c, card('knife', 1, 'ally')]));
      const hits = sim.frames
        .flatMap((f) => f.hits)
        .filter((h) => h.sourceUid === id);
      if (id === 'knife' || id === 'brick') {
        const damage = hits.filter((h) => h.kind === 'damage');
        assert.equal(damage[0].raw, d.effects[0].value);
        assert.equal(
          damage[2].raw - damage[0].raw,
          quality
            ? id === 'knife'
              ? quality === 2
                ? 12
      ...
      if (id === 'wire' || id === 'shelter')
        assert.equal(
          hits.some((h) => h.kind === 'charge'),
          quality > 0,
        );
    }
  for (const c of CARDS)
    for (const quality of [0, 1, 2]) {
      ...
      assert.ok(
        d.summary !== undefined &&
          d.effects.every((e) => Number.isFinite(e.value)),
      );
    }
});

test('phase1 identification four states, carried scanner, atomic fees and base free', () => {
  const s = act(start(), { type: 'enter', floor: 1 });
  s.items.push(makeItem('unknown', 'fuse', 'physical'));
  assert.match(identificationState(s).reason, /未解锁/);
  reject(s, { type: 'scan', id: 'unknown' }, /Lv.3/);
  s.level = 3;
  assert.match(identificationState(s).reason, /未携带/);
  reject(s, { type: 'scan', id: 'unknown' }, /未携带/);
  ...
  s.charges = 0;
  assert.match(identificationState(s).reason, /电荷不足/);
  reject(s, { type: 'scan', id: 'unknown' }, /电荷不足/);
  s.charges = 2;
  assert.equal(identificationState(s).reason, '可鉴定');
  ...
  const count = b.charges;
  b = act(b, { type: 'scan', id: 'base-unknown' });
  assert.equal(b.charges, count);
  assert.equal(b.items.at(-1).zone, 'warehouse');
});

test('phase1 placement covers sizes, locks, cancellation, full bag, unequal swaps and invalid multi-overlap', () => {
  let s = start();
  const original = structuredClone(s);
  assert.equal(previewPlacement(s, 'starter-knife', 0).allowed, true);
  assert.deepEqual(s, original);
  s = act(s, { type: 'place', id: 'starter-knife', at: 0 });
  assert.equal(s.items.find((x) => x.uid === 'starter-wire').at, 6);
  reject(s, { type: 'place', id: 'starter-shelter', at: 2 }, /跨路/);
  reject(s, { type: 'place', id: 'starter-wire', at: 1 }, /解锁/);
  s.level = 6;
  ...
  s = act(s, { type: 'place', id: 'starter-shelter', at: 6 }); // two cells swap with one cell; old lane has space.
  assert.equal(s.items.find((x) => x.uid === 'starter-wire').at, 3);
  reject(s, { type: 'unequip', id: 'starter-wire' }, /背包/);
  s = act(s, { type: 'place', id: 'starter-wire', at: 4 });
  assert.equal(s.items.find((x) => x.uid === 'starter-wire').at, 4);
  // Three-cell candidate would overlap current knife: use fresh board for size boundaries.
  let t = start();
  t.level = 6;
  t.items = [
    card('battery', 0, 'three'),
    card('shelter', 3, 'two'),
    card('knife', 6, 'one'),
  ];
  ...
  t.items.push(card('wire', 4, 'blocker'));
  reject(t, { type: 'place', id: 'three', at: 3 }, /多张/);
  reject(t, { type: 'place', id: 'three', at: 7 }, /跨路/);
  let u = start();
  u.items.push(makeItem('candidate', 'fuse', 'card'));
  u = act(u, { type: 'place', id: 'candidate', at: 0 });
  assert.equal(u.items.find((x) => x.uid === 'starter-wire').zone, 'bag');
...
test('phase1 board position survives legacy migration, save, preview, opening, replay; no node/cost changes', () => {
  ...
  s = act(s, { type: 'place', id: 'starter-wire', at: 0 });
  ...
  reject(s, { type: 'place', id: 'starter-wire', at: 6 }, /战斗/);
...
test('phase1 refinement consumes named deterministic copy once, keeps primary; sleep consumes actual supply', () => {
  let s = start();
  s.level = 2;
  s.items.push(makeItem('copy', 'wire', 'card'));
  const count = s.items.length;
  s = act(s, { type: 'refine', id: 'starter-wire' });
  assert.equal(s.items.length, count - 1);
  assert.ok(!s.items.some((x) => x.uid === 'copy'));
  assert.equal(s.items.find((x) => x.uid === 'starter-wire').quality, 1);
  reject(s, { type: 'refine', id: 'starter-wire' }, /同名/);
...
test('phase1 same resources / cards, only position changes produce explainable events', () => {
  ...
  const pair = simulateDuel(duel([card('fuse', 0), card('knife', 1)]));
  const hit = pair.frames
    .flatMap((f) => f.hits)
    .find((h) => h.kind === 'charge');
  assert.equal(hit.targetUid, 'knife');
  assert.match(hit.targetName, /水果刀/);
  assert.ok(hit.value > 0);
  assert.match(
    formatHit(duel([card('fuse', 0), card('knife', 1)]), hit, 6),
    /我方.*引信线.*我方.*水果刀/,
  );
...
test('phase1 guaranteed low-cost natural acquisition before second departure, no injected ideal inventory', () => {
  ...
  assert.ok(
    simulateDuel(nextBattle)
      ...
      .some(
        (h) =>
          h.sourceUid === offer.uid &&
          h.targetUid === 'starter-knife' &&
          h.value > 0,
      ),
  );
