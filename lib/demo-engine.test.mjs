...
const doAct = (s, type, rest = {}) => {
  const next = act(s, { type, ...rest });
  if (type === 'resolve' && next.loot) {
    const uid = next.loot.item.uid;
    return act(claimReward(next), { type: 'drop', id: uid });
  }
  return next;
};
...
function gather(s) {
  for (const x of currentFloor(s).stock.filter(
    (x) =>
      (['resource', 'tool'].includes(x.type) &&
        ['material', 'supply', 'scrap'].includes(x.id)) ||
      x.id === 'apple' ||
      (x.type === 'physical' &&
        ['damage', 'corrode'].includes(cardDef(x.id).kind)),
  )) {
    try {
...
function explore(s) {
  for (
    ...
    if (s.interaction === 'search') {
      ...
      s = doAct(
        ...
        tool
          ? {
              id: tool.uid,
              choice: fieldTask(s.seed, s.floor, n).answer,
              at: 0,
            }
          : { choice: -1 },
      );
    ...
    else if (n === 'antechamber') s = doAct(s, 'approach-guardian');
...
test('demo: natural five-floor journey completes through real battles, growth, sleep, save/restore and extraction', () => {
  for (const seed of [12, 123, 711]) {
    ...
    for (let floor = 1; floor <= 5; floor++) {
      ...
      for (const id of ['starter-gapblade', 'starter-nailer']) {
      ...
      if (floor < 5) {
        for (const x of [...s.items].filter((x) => x.type === 'physical')) {
          if (s.level < 4 && !itemCount(s, 'scanner', false)) {
            if (s.material < 12 || (s.shopDay === s.day && s.shopBought >= 2))
              continue;
            s = doAct(s, 'buy-scanner');
          }
          if (s.level >= 4 && s.material < 2) continue;
        ...
        // Keep room for the next expedition after the enlarged card pool adds
        // multi-cell items that cannot all fit on the active board.
        for (const x of [...s.items].filter(
          (x) => x.zone === 'bag' && x.id !== 'apple',
        )) {
          try {
            s = doAct(s, 'move', { id: x.uid, to: 'warehouse' });
          } catch {}
        }
      }
...
test('demo: item zones conserve identities and volume; multi-cell cards cannot cross lanes or use locked cells', () => {
  ...
  assert.throws(
    () => doAct(s, 'move', { id: 'starter-nailer', to: 'board', at: 2 }),
    /跨路/,
  );
  assert.deepEqual(s, initial);
  s = doAct(s, 'move', { id: 'starter-nailer', to: 'bag' });
  assert.equal(s.items.find((x) => x.uid === 'starter-nailer').zone, 'bag');
  s = doAct(s, 'move', { id: 'starter-nailer', to: 'safe' });
...
test('demo: scanner consumes the carried tool, rarity locks once, scanning is atomic', () => {
...
test('demo: level-four identification is permanent; weather is unavailable and production obeys daily limits', () => {
  let s = start();
  s.level = 4;
  s = migrateCargo(s);
...
test('demo: deterministic immutable combat uses individual rarity/levels and emits owner effect events', () => {
  ...
  assert.ok(validSave(s));
  const packed = doAct(s, 'move', { id: 'starter-gapblade', to: 'bag' });
  assert.deepEqual(packed.duel, s.duel);
  assert.deepEqual(simulateDuel(packed.duel), r);
  s = doAct(s, 'resolve');
...
test('demo: corrupt versions, invalid levels, overlapping boards and incomplete snapshots are rejected', () => {
  ...
  s.items.find((x) => x.uid === 'starter-gapblade').at = 0;
...
test('demo: all installed facilities change real shared resources and preparation affects the next search', () => {
  ...
  for (const [id, key] of Object.entries(fixtures)) {
    ...
    s = doAct(s, 'build', { id });
    const read = (state) =>
      ['supply', 'fuel', 'scanner'].includes(key)
