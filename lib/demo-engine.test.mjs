...
function explore(s) {
  for (
    let guard = 0;
    guard < 20 && s.phase === 'floor' && !s.objective;
    guard++
  ) {
    const n = currentNode(s);
    if (n === 'event') s = doAct(s, 'event');
  ...
  for (const seed of [12, 123, 711]) {
    ...
    for (let floor = 1; floor <= 10; floor++) {
      ...
      for (const id of ['starter-knife', 'starter-wire']) {
        ...
        if (card.level < 5 && s.material >= 3 + card.level * 2)
          s = doAct(s, 'grow', { id });
        card = s.items.find((x) => x.uid === id);
        if (card.quality < 1 && s.material >= 5) s = doAct(s, 'refine', { id });
      }
      if (s.level < 4 && s.material >= 5 + s.level) s = doAct(s, 'upgrade');
      ...
      for (const x of currentFloor(s).stock)
        if (x.type === 'resource' && ['material', 'supply'].includes(x.id)) {
          try {
            s = doAct(s, 'pickup', { id: x.uid });
          } catch {}
        }
      for (const x of [...currentFloor(s).stock].filter(
        (x) => x.type === 'physical' && ['knife', 'wire'].includes(x.id),
      )) {
        try {
          s = doAct(s, 'pickup', { id: x.uid });
        } catch {}
...
test('demo: item zones conserve identities and volume; multi-cell cards cannot cross lanes or use locked cells', () => {
  ...
  assert.throws(() => doAct(s, 'move', { id: 'scanner', to: 'safe' }), /容量/);
...
test('demo: retreat does not count objective, persistent stock is conserved and same-floor retry is allowed', () => {
  ...
  assert.ok(!currentFloor(s).stock.some((i) => i.uid === x.uid));
  assert.throws(() => doAct(s, 'enter', { floor: 2 }));
  s = doAct(s, 'sleep');
  assert.throws(() => doAct(s, 'enter', { floor: 1 }));
  s = doAct(s, 'enter', { floor: 2 });
...
test('demo: free observatory and all facilities obey construction and daily anti-rebuild rules', () => {
  let s = start();
  s = doAct(s, 'build', { id: 'weather' });
  assert.equal(s.material, 10);
  const power = s.power;
  s = doAct(s, 'facility', { id: 'weather' });
  assert.equal(s.power, power);
  assert.ok(s.forecast);
  s = doAct(s, 'remove', { id: 'weather' });
  s = doAct(s, 'build', { id: 'weather' });
  assert.throws(() => doAct(s, 'facility', { id: 'weather' }));
  s = doAct(s, 'sleep');
  s = doAct(s, 'facility', { id: 'weather' });
  assert.ok(s.forecast);
});
test('demo: deterministic immutable combat uses individual rarity/levels and emits owner effect events', () => {
  let s = start();
  s = doAct(s, 'enter', { floor: 1 });
  while (currentNode(s) !== 'guardian') {
    const n = currentNode(s);
    s = doAct(
      s,
      n === 'puzzle' ? 'puzzle' : n === 'event' ? 'event' : 'skip',
      n === 'puzzle' ? { choice: puzzle(s).answer } : {},
    );
  }
  s = doAct(s, 'fight');
