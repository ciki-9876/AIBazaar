...
test('cargo: orientation changes occupied cells, overlaps and boundary crossings reject', () => {
  assert.deepEqual(
...
test('cargo: rejected pickup or purchase preserves inventory, stock and gold; rotation survives save', () => {
  let s = start();
  ...
  assert.equal(s.items.find((x) => x.uid === 'starter-shelter').rotated, true);
