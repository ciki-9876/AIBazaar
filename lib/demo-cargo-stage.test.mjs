...
test('cargo: rejected pickup or purchase preserves inventory, stock and gold; horizontal placement survives save', () => {
  let s = start();
  s.items = s.items.filter((x) => x.zone === 'board' || x.id === 'supply');
  s = act(s, {
    type: 'move',
    id: 'starter-nailer',
    to: 'bag',
  ...
  assert.equal(s.items.find((x) => x.uid === 'starter-nailer').rotated, false);
...
test('defeat: normal and elite lose stamina only; boss loses life and ordinary inventory', () => {
  for (const node of ['patrol', 'elite', 'guardian']) {
    ...
    if (node === 'guardian') {
      ...
      assert.equal(result.phase, 'floor');
      assert.equal(result.floor, 10);
      assert.equal(result.node, s.node + 1);
      assert.equal(result.material, s.material);
      assert.equal(result.objective, false);
      assert.deepEqual(result.bots, s.bots);
      assert.equal(result.quota, life);
      assert.equal(result.stamina, Math.max(0, stamina - 38));
      assert.deepEqual(
...
test('survivor AI defeat costs life and forces recovery even with normal stage', () => {
  let s = act(start(), { type: 'enter', floor: 10 });
  s.node = currentFloor(s).nodes.indexOf('guardian');
  s.encounter = 1;
  ...
  const result = act(s, { type: 'resolve' });
  assert.equal(result.phase, 'base');
  assert.equal(result.quota, s.quota - 3);
  assert.equal(result.floor, 0);
  assert.equal(result.items.filter((x) => x.zone === 'bag').length, 0);
  assert.ok(validSave(result));
});

test('victory rewards preserve enemy card properties and cannot be claimed twice', () => {
  let s = act(start(), { type: 'enter', floor: 1 });
  s.encounter = null;
  s.node = currentFloor(s).nodes.indexOf('elite');
  s = act(s, { type: 'fight' });
  const enemies = structuredClone(s.duel.enemy),
    gold = s.material;
  s.duel.maxHp = [10000, 1];
  s = act(s, { type: 'resolve' });
  assert.ok(s.loot);
  const pending = JSON.parse(JSON.stringify(s));
  assert.ok(validSave(pending));
  s = claimReward(pending);
  assert.throws(() => act(s, { type: 'claim-loot' }));
  assert.throws(() => act(s, { type: 'resolve' }));
  const trophy = s.items.find((x) => x.uid.startsWith('trophy-'));
  assert.ok(trophy);
  assert.ok(
    enemies.some(
      (x) =>
        x.id === trophy.id &&
        x.level === trophy.level &&
        x.quality === trophy.quality &&
        x.rarity === trophy.rarity,
    ),
  );
  assert.equal(s.material, gold + 2);
  assert.ok(validSave(s));
  ...
  assert.ok(s.loot); // A genuinely new victory grants a new weapon, not duplicate gold.
  assert.equal(s.material, gold + 2);
});

test('node movement costs energy and insufficient energy rejects without changing inventory', () => {
  const s = act(start(), { type: 'enter', floor: 1 });
  s.node = currentFloor(s).nodes.indexOf('merchant');
  s.stamina = 10;
  const next = act(s, { type: 'skip' });
  assert.equal(next.stamina, 7);
  assert.equal(next.node, s.node + 1);
  s.stamina = 2;
  const copy = structuredClone(s);
  assert.throws(() => act(s, { type: 'skip' }), /3 精力/);
  assert.deepEqual(s, copy);
});

test('full backpack keeps a persistent unclaimed trophy; failed claim is atomic', () => {
  let s = act(start(), { type: 'enter', floor: 1 });
  s.items = s.items.filter((x) => x.zone !== 'bag');
  for (let i = 0; i < bagCap(s); i++)
    s.items.push(makeItem(`full-${i}`, 'apple', 'tool', 'bag'));
  s.node = currentFloor(s).nodes.indexOf('elite');
  s = act(s, { type: 'fight' });
  s.duel.maxHp = [10000, 1];
  const ids = s.items.map((x) => x.uid),
    gold = s.material;
  s = act(s, { type: 'resolve' });
  assert.equal(s.material, gold + 2);
  assert.deepEqual(
  ...
  assert.throws(() => act(s, { type: 'claim-loot' }));
  assert.deepEqual(s, before);
  for (let i = 0; i < 3; i++) s = act(s, { type: 'drop', id: `full-${i}` });
  const uid = s.loot.item.uid;
  s = act(s, { type: 'claim-loot' });
  assert.ok(s.items.some((x) => x.uid === uid));
