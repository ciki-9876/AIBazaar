...
function go(s, target) {
  for (let i = 0; i < 20 && currentNode(s) !== target; i++) {
      ...
      type:
        n === 'puzzle'
          ...
          : ['event', 'rest', 'hazard'].includes(n)
            ? n
            : 'skip',
      choice: n === 'puzzle' ? puzzle(s).answer : 0,
    });
  }
  assert.equal(currentNode(s), target);
  return s;
}
...
test('v1.1: search holds position, charges once, persists save and exits only on explicit next', () => {
  ...
  const item = currentFloor(s).stock[0];
  ...
  assert.throws(() =>
    act(s, { type: 'pickup', id: currentFloor(s).stock[0].uid }),
  );
});
...
test('v1.1: legacy saved routes stay valid while untouched floors adopt new content on entry', () => {
  ...
  assert.equal(currentFloor(s).routeVersion, 2);
...
test('v1.1: all emitted card projectiles carry real source and target entities, charge lands on cards', () => {
  ...
  const ids = new Set(
    [...player, ...enemy].map((x) => x.uid).concat(['host-0', 'host-1']),
  );
  ...
  assert.ok(effects.some((h) => h.kind === 'heal' && h.targetUid === 'host-0'));
